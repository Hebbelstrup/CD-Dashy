...
def Denature(x, An, Bn, Ad, Bd, dS, dH): # Function to be fitted to data 
    return (An+Bn*(x+273)+(Ad+Bd*(x+273))*np.exp((dS*(x+273)-dH)/(0.00831*(x+273))))/(1+np.exp((dS*(x+273)-dH)/(0.00831*(x+273))))



app.layout = html.Div(id ='parent', children = [html.H1(id = 'H1', children = 'CD Plotter', style = {'textAlign':'center','marginTop':40, 'marginBottom':40}),\
                                                ...
                                                dcc.Checklist(id ='Converter',options = [{'label':'Convert to MRE','value':'Convert'}],style={'display':'inline-block'}),
...
@app.callback(Output('placeholder','children'),[Input('Concentration', 'value'), Input('Peptide-bonds','value')],)
...
def plot_data(contents, filename, convert,C,P,files,clicks, An, Bn, Ad, Bd, dS, dH):
    ...
    for data,filename in zip(contents,filename):
        if f'{filename}' in files:
            ...
            fig.add_trace(go.Scatter(x=x, y=df['Sat'],line = {'width': 1,'color':colors[i]},showlegend=False), row=2,col=1)
            i += 1 
            ...
            if clicks:
                try:
                    ...
                    fig.add_trace(go.Scatter(x=x, y=Denature(x.to_numpy(),par[0],par[1],par[2],par[3],par[4],par[5]), 
                                         line = {'width': 1,'color':colors[i],'dash':'dash'},name=f'An = {par[0]:.3} +- {perr[0]:.3}\
...
def update_file_select(filenames):
    return [[i for i in filenames],[i for i in filenames]] # returns first for options and then for values. its the same in this case. This type of callback needs [[],[]] - List of lists. 


###### START OF FITTING #######

@app.callback(Output('An','style'),Output('Bn','style'),Output('Ad','style'),Output('Bd','style'),Output('dS','style'),Output('dH','style'),Output('fit','style'),
